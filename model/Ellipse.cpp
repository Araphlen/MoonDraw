...
Ellipse::Ellipse() :Figure(ELLIPSE_SHAPE, MyRgb(0,0,0), MyRgb(255,255,255),1, false) {
    m_topLeft = Point(0,0);
    m_h=10;
    m_w=20;
...
Ellipse::Ellipse(const Point &topLeft, int h, int w,const MyRgb &penColor, const MyRgb &brushColor,int penSize,bool isTransparent) :Figure(ELLIPSE_SHAPE,penColor,brushColor,penSize,isTransparent) {
    m_topLeft = topLeft;
    m_h = h;
    m_w = w;
    m_bottomRight = Point(m_w+ m_topLeft.getX(), m_h + m_topLeft.getY());
}
Ellipse::Ellipse(const Point &topLeft, const Point &bottomRight,const MyRgb &penColor, const MyRgb &brushColor,int penSize,bool isTransparent) :Figure(ELLIPSE_SHAPE,penColor,brushColor,penSize,isTransparent) {
    m_topLeft = topLeft;
