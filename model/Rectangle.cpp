...
Rectangle::Rectangle() :Figure(RECT_SHAPE,"#000000","#FFFFFF",1) {
    m_topLeft = Point(0,0);
    m_h=10;
    m_w=20;
...
Rectangle::Rectangle(const Point &topLeft, int h, int w,const std::string& penColor,const std::string& brushColor,int penSize) :Figure(RECT_SHAPE,penColor,brushColor,penSize) {
    m_topLeft = topLeft;
    m_h = h;
    m_w = w;
    m_bottomRight = Point(m_w+ m_topLeft.getX(), m_h + m_topLeft.getY());
}
Rectangle::Rectangle(const Point &topLeft, const Point &bottomRight,const std::string& penColor,const std::string& brushColor,int penSize) :Figure(RECT_SHAPE,penColor,brushColor,penSize) {
    m_topLeft = topLeft;
